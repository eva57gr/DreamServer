...
export default function Settings() {
  ...
  const handleRollback = async () => {
    try {
      ...
      const payload = backupId ? { action: 'rollback', backup_id: backupId } : { action: 'rollback' }
      const result = await postUpdateAction(payload)
      if (result.success) {
        setNotice({ type: 'warn', text: `Rollback started${backupId ? ` (${backupId})` : ''}.` })
      } else {
        setNotice({ type: 'danger', text: 'Rollback command failed. Check logs for details.' })
      }
      await refreshReadiness()
    } catch (err) {
      setNotice({ type: 'danger', text: `Rollback failed: ${err.message}` })
    } finally {
      setUpdateBusy(false)
    }
  }
  ...
  const updateAvailable = Boolean(updateReadiness?.update_available)
          ...
          <div className="space-y-4">
            <div>
              <p className="text-white">
                {updateAvailable
                  ? `Update available: ${updateReadiness?.current || version?.version} → ${updateReadiness?.latest}`
                  : 'You are on the latest release'}
              </p>
              <p className="text-sm text-zinc-500">
                Last checked: {updateReadiness?.checked_at || updateReadiness?.last_check || 'unknown'}
              </p>
              <p className="text-sm text-zinc-500">
                Last updated: {updateReadiness?.last_update || 'unknown'}
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                disabled={updateBusy || !updateSystemAvailable || !rollbackAvailable}
